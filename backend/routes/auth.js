...
router.post("/google", async (req, res) => {
  try {
    ...
    if (accessToken) {
      ...
      const uri =
        redirectUri ||
        "https://personal-finance-tracker-sovit.netlify.app/auth/google/callback";
      const tokens = await exchangeGoogleCode(code, uri);
      if (tokens.error)
        return res
          .status(400)
          .json({ error: tokens.error_description || "Google auth failed." });
      googleUser = await getGoogleUserInfo(tokens.access_token);
    } else {
      return res
        .status(400)
        .json({ error: "accessToken or code is required." });
    }
...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    const frontendUrl =
      ...
      "https://personal-finance-tracker-sovit.netlify.app";
